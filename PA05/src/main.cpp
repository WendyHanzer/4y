...
////
    ...
    for (unsigned int i = 0 ; i < myMesh.m_Entries.size() ; i++) 
    {
        glBindBuffer(GL_ARRAY_BUFFER, myMesh.m_Entries[i].VB);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0);
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid*)12);
        glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid*)20);

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, myMesh.m_Entries[i].IB);

        const unsigned int MaterialIndex = myMesh.m_Entries[i].MaterialIndex;
/*
        if (MaterialIndex < myMesh.m_Textures.size() && myMesh.m_Textures[MaterialIndex]) {
            myMesh.m_Textures[MaterialIndex]->Bind(GL_TEXTURE0);
        }
*/
        glDrawElements(GL_TRIANGLES, myMesh.m_Entries[i].NumIndices, GL_UNSIGNED_INT, 0);
    }
...
bool initialize( char* objectFile )
{
    ...
    myMesh.m_Entries.resize(scene->mNumMeshes);
    myMesh.m_Textures.resize(scene->mNumMaterials);
    ...
    for (unsigned int i=0; i<myMesh.m_Entries.size(); i++)
    {
        ...
        myMesh.m_Entries[i].MaterialIndex = myaiMesh->mMaterialIndex;
        ...
        myMesh.m_Entries[i].NumIndices = Indices.size();
        glGenBuffers(1, &myMesh.m_Entries[i].VB);
        glBindBuffer(GL_ARRAY_BUFFER, myMesh.m_Entries[i].VB);
        glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * Vertices.size(), &Vertices[0], GL_STATIC_DRAW);
        
        glGenBuffers(1, &myMesh.m_Entries[i].IB);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, myMesh.m_Entries[i].IB);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * myMesh.m_Entries[i].NumIndices, &Indices[0], GL_STATIC_DRAW);
    ...
    for (unsigned int i=0; i<scene->mNumMaterials; i++)
    {
        ...
        if (myMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0)
        {
            ...
            if (myMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS)
            {
                ...
                myMesh.m_Textures[i] = new Texture(GL_TEXTURE_2D, FullPath.c_str());

                if (!m_Textures[i]->Load()) 
                {
                    printf("Error loading texture '%s'\n", FullPath.c_str());
                    delete myMesh.m_Textures[i];
                    myMesh.m_Textures[i] = NULL;
                    Ret = false;
                }
                else 
